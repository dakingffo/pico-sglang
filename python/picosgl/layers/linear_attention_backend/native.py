...
from .base import BaseLinearAttentionBackend
...
def _prepare(
    ...
    query, key = _expand_gva(query, key, value)
    beta = beta.sigmoid()
    gate = -A_log.float().exp() * F.softplus(gate.float() + dt_bias)
...
class NativeLinearAttentionBackend(BaseLinearAttentionBackend):
    def _prefill(
        ...
        query, key, value, gate, beta = _prepare(
            query, key, value, gate, beta, A_log, dt_bias
        )
